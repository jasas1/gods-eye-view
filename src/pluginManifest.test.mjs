...
test('normalizePluginManifest: string entries pass through under the client rules', () => {
  const { rootEntries, externals, clientManifest, errors } = normalizePluginManifest(
    { plugins: ['./plugins/hello-layer/index.js', '/x.js', './plugins/../core/index.js', 7] },
    { root: process.cwd() },
  );
...
test('normalizePluginManifest: object entries need name/dir/entry rules', () => {
  ...
  try {
    ...
    assert.deepEqual(externals[0], { name: 'ext-demo', dir: extRoot, entry: 'index.js' });
...
test('normalizePluginManifest: bad shape errors', () => {
  assert.match(normalizePluginManifest(null, { root: process.cwd() }).errors[0], /object with a "plugins" array/);
  assert.match(normalizePluginManifest({ plugins: 'x' }, { root: process.cwd() }).errors[0], /must be an array/);
});
...
test('safeJoinInside: rejects "..", encoded "%2e%2e", backslash, and absolute traversal', () => {
  ...
  try {
    ...
    assert.equal(safeJoinInside(dir, 'sub/a.js'), realpathSync(path.join(dir, 'sub', 'a.js')));
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test('safeJoinInside: rejects a symlink pointing outside the dir', () => {
  ...
  try {
    ...
    assert.equal(safeJoinInside(dir, 'inside.js'), realpathSync(path.join(dir, 'real.js')));
  } finally {
    rmSync(dir, { recursive: true, force: true });
...
function fakeRes() {
  return {
    ...
    end(body) { this.ended = true; this.body = body === undefined ? null : body; },
  };
}
...
test('middleware: serves /gev.plugins.json normalized (no absolute paths)', async () => {
  ...
  try {
    ...
    assert.ok(!String(r.res.body).includes(ext), 'client manifest must not leak the absolute dir');
  } finally {
    rmSync(ext, { recursive: true, force: true });
    cleanup();
  }
...
test('middleware: 404 for a traversal attempt', async () => {
  ...
  try {
    ...
    const invoke = buildHandler(root);
    for (const url of ['/plugins/ext-demo/../secret.txt', '/plugins/ext-demo/%2e%2e/secret.txt']) {
...
test('gevPluginsVitePlugin: closeBundle copies root + external plugins and writes the client manifest', () => {
  ...
  try {
    ...
    writeFileSync(path.join(root, 'plugins', 'hello-layer', 'index.js'), 'export default 1;');
    writeFileSync(path.join(root, 'plugins', 'hello-layer', '.dotfile'), 'skip');
    ...
    assert.ok(existsSync(path.join(outDir, 'gev.plugins.json')), 'dist manifest written');
    const distManifest = JSON.parse(readFileSync(path.join(outDir, 'gev.plugins.json'), 'utf8'));
    assert.deepEqual(distManifest.plugins, [
      './plugins/hello-layer/index.js',
      './plugins/ext-demo/index.js',
    ]);
    assert.ok(existsSync(path.join(outDir, 'plugins', 'hello-layer', 'index.js')));
    assert.ok(existsSync(path.join(outDir, 'plugins', 'ext-demo', 'index.js')));
    assert.ok(existsSync(path.join(outDir, 'plugins', 'ext-demo', 'points.js')));
    assert.ok(!existsSync(path.join(outDir, 'plugins', 'ext-demo', 'node_modules')), 'node_modules skipped');
    assert.ok(!existsSync(path.join(outDir, 'plugins', 'hello-layer', '.dotfile')), 'dotfile skipped');
  } finally {
...
test('gevPluginsVitePlugin: configureServer registers a middleware', () => {
  ...
  try {
    const plugin = gevPluginsVitePlugin();
    let registered = null;
    plugin.configureServer({ middlewares: { use(fn) { registered = fn; } }, config: { root } });
