  ...
  for (const entry of raw) {
    ...
    if (entry && typeof entry === 'object' && !Array.isArray(entry)) {
      ...
      if (typeof name !== 'string' || !NAME_RE.test(name)) {
        errors.push(`plugin entry name must match /^[a-z0-9-]+$: ${String(name)}`);
        continue;
      }
      if (names.has(name)) {
      ...
      if (typeof dir !== 'string' || !path.isAbsolute(dir)) {
        errors.push(`plugin entry dir must be an absolute path: ${String(dir)}`);
        continue;
      }
      let dirStat;
      try {
      ...
      if (!dirStat || !dirStat.isDirectory()) {
        errors.push(`plugin entry dir does not exist or is not a directory: ${dir}`);
        continue;
      }
      if (typeof entryName !== 'string' || entryName.length === 0
        || entryName.includes('/') || entryName.includes('\\') || !ENTRY_RE.test(entryName)) {
        errors.push(
...
export function readPluginManifest(root) {
  ...
  try {
    ...
    console.warn(`[gev plugins] malformed manifest JSON: ${err && err.message ? err.message : err}`);
    return {
      rootEntries: [],
      externals: [],
...
function buildCopyPlugins(root, outDir) {
  ...
  for (const entry of manifest.rootEntries) {
    ...
    if (!srcStat || !srcStat.isDirectory()) {
      console.warn(`[gev plugins] root plugin directory not found, skipped: ${srcDir}`);
      continue;
    }
    copyPluginDir(srcDir, destDir);
    console.log(`[gev plugins] copied root plugin ${relDir} -> ${path.relative(outDir, destDir)}`);
  }
  ...
  for (const external of manifest.externals) {
    ...
    console.log(`[gev plugins] copied external plugin ${external.name} -> plugins/${external.name}`);
  }
