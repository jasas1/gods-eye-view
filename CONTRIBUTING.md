...
You can also ship a layer as an **external runtime plugin** listed in a `gev.plugins.json` manifest, with no edits to core files — see [docs/PLUGINS.md](docs/PLUGINS.md) for the manifest shape, the plugin contract, and the demo `hello-layer` plugin. This is an opt-in, dev-only prototype; the app boots unchanged when no manifest is present.
