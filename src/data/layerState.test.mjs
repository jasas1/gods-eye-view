...
test('extendLayerStateRegistry: plugin layers round-trip through encode/decode', () => {
  try {
    const extras = [{ id: 'hello-layer', token: 'p', disposition: 'enabled-only' }];
    ...
    // 'p' (hello-layer) and 'e' (earthquakes) must both be present, base order.
    assert.match(params.get('l'), /p/);
...
test('extendLayerStateRegistry: token/id collision throws', () => {
  try {
    assert.throws(
      () => extendLayerStateRegistry([{ id: 'earthquakes', token: 'z', disposition: 'enabled-only' }]),
      /Duplicate layer-state id: earthquakes/,
    );
    assert.throws(
      () => extendLayerStateRegistry([{ id: 'plugin-x', token: 'e', disposition: 'enabled-only' }]),
