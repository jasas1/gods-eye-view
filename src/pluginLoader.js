 ...
 * caller (src/main.js) then registers the layers into the sealed
 * DataLayerManager before finalization.
 *
 * Design constraints:
 ...
 */
export function validatePluginDescriptor(descriptor, { existingLayerIds, existingTokens }) {
  const errors = [];
  ...
  for (const layer of layers) {
    ...
    for (const fn of ['init', 'enable', 'disable', 'update', 'destroy', 'getStats']) {
      if (typeof layer[fn] !== 'function') missing.push(fn);
    }
    if (missing.length) {
      errors.push(`plugin layer missing interface fields: ${missing.join(', ')}`);
      continue;
    }
    if (typeof layer.id !== 'string' || !ID_RE.test(layer.id)) {
      errors.push(`plugin layer id must match /^[a-z0-9-]+$/: ${String(layer.id)}`);
      continue;
    }
    if (existingLayerIds.has(layer.id)) {
      errors.push(`plugin layer id collides with an existing layer id: ${layer.id}`);
    ...
    if (!Array.isArray(share)) {
      ...
      for (const entry of share) {
        ...
        if (typeof entry.id !== 'string' || !layerIds.has(entry.id)) {
          errors.push(`plugin share id must reference one of the plugin's layers: ${String(entry?.id)}`);
        }
        if (typeof entry.token !== 'string' || !TOKEN_RE.test(entry.token)) {
          errors.push(`plugin share token must be a single [a-z0-9] char: ${String(entry?.token)}`);
          continue;
        }
        if (existingTokens.has(entry.token)) {
          errors.push(`plugin share token collides with an existing token: ${entry.token}`);
        } else if (shareTokens.has(entry.token)) {
          errors.push(`plugin share token duplicated within the plugin: ${entry.token}`);
        } else {
...
export async function loadPlugins({
  ...
  try {
    ...
    errors.push({ spec: manifestUrl, error: `fetch failed: ${describeError(error)}` });
    console.error(`[gev plugins] manifest fetch failed: ${describeError(error)}`);
  ...
  try {
    ...
    errors.push({ spec: manifestUrl, error: `manifest read failed: ${describeError(error)}` });
    console.error(`[gev plugins] manifest read failed: ${describeError(error)}`);
    return { plugins, shareEntries, errors };
  }
  ...
  try {
    ...
    errors.push({ spec: manifestUrl, error: `malformed manifest JSON: ${describeError(error)}` });
    console.error(`[gev plugins] malformed manifest JSON: ${describeError(error)}`);
    return { plugins, shareEntries, errors };
  }

  const { plugins: specs, errors: manifestErrors } = validatePluginManifest(parsed);
  ...
  for (const spec of specs) {
    ...
    try {
      ...
      console.error(`[gev plugins] import failed for ${spec}: ${describeError(error)}`);
      continue;
    }
    const createPlugin = moduleNs?.default;
    if (typeof createPlugin !== 'function') {
      errors.push({ spec, error: 'module default export must be a createPlugin function' });
      console.error(`[gev plugins] ${spec}: default export must be a createPlugin function`);
      continue;
    }
    let descriptor;
    try {
      ...
      errors.push({ spec, error: `createPlugin threw: ${describeError(error)}` });
      console.error(`[gev plugins] ${spec}: createPlugin threw: ${describeError(error)}`);
      continue;
    }
